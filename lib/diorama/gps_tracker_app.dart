...
class _GpsTrackerPageState extends State<GpsTrackerPage> with WidgetsBindingObserver {
  ...
  double _terrainRotation = math.pi / 4;
        ...
        return ListView.separated(
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            return ListTile(
              ...
              title: Text('Sessão ${session.shortId}'),
              subtitle: Text('${session.pointCount} pontos · ${session.createdAt.toLocal()}'),
  ...
  void _setTerrainRotation(double value) {
    setState(() => _terrainRotation = value);
    _centerDioramaOnLastPoint();
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          PositionDioramaCard(
            ...
            terrainRotation: _terrainRotation,
            onTerrainRotationChanged: _setTerrainRotation,
...
class PositionDioramaCard extends StatelessWidget {
  const PositionDioramaCard({
    ...
    required this.terrainRotation,
    required this.onTerrainRotationChanged,
  ...
  final double terrainRotation;
  final ValueChanged<double> onTerrainRotationChanged;
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Slider(
                    value: terrainRotation,
                    min: 0,
                    max: math.pi * 2,
                    onChanged: onTerrainRotationChanged,
                  ),
                ),
                Text('${(terrainRotation * 180 / math.pi).round()}°'),
              ],
            ),
            const SizedBox(height: 12),
            Center(
                  ...
                  child: DecoratedBox(
                    ...
                    child: InteractiveViewer(
                      ...
                      child: SizedBox(
                        ...
                        child: CustomPaint(
                          painter: DioramaSessionPainter(points: points, rotation: terrainRotation),
                        ),
                      ),
                    ),
                  ),
...
class DioramaViewport {
  ...
  static Offset projectPoint({
    ...
    return projectPerspectiveGround(metersX: metersX, metersZ: metersZ, rotation: rotation);
  }

  static Offset projectPerspectiveGround({
    ...
    final double rotatedX = x * math.cos(rotation) - z * math.sin(rotation);
    final double rotatedZ = x * math.sin(rotation) + z * math.cos(rotation);

    return Offset(
      canvasCenter.dx + (rotatedX - rotatedZ) * 0.72,
      canvasCenter.dy + (rotatedX + rotatedZ) * 0.36,
    );
  }

  static Matrix4 centeredInitialTransform({
    double scale = 1.0,
    double rotation = math.pi / 4,
  }) {
    // rotation é aceito por compatibilidade com chamadas que recentralizam
    // após alterar a rotação do terreno. O centro inicial permanece o centro
    // lógico do canvas.
...
class DioramaSessionPainter extends CustomPainter {
  DioramaSessionPainter({required this.points, required this.rotation});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final List<Offset> projected = points
        .map((GpsPoint point) => DioramaViewport.projectPoint(points: points, point: point, rotation: rotation))
        .toList();
  ...
  void _drawSceneBase(Canvas canvas, Offset center, double tileW, double tileH) {
    ...
    final Offset northWest = DioramaViewport.projectPerspectiveGround(metersX: -halfMeters, metersZ: halfMeters, rotation: rotation);
    final Offset northEast = DioramaViewport.projectPerspectiveGround(metersX: halfMeters, metersZ: halfMeters, rotation: rotation);
    final Offset southEast = DioramaViewport.projectPerspectiveGround(metersX: halfMeters, metersZ: -halfMeters, rotation: rotation);
    final Offset southWest = DioramaViewport.projectPerspectiveGround(metersX: -halfMeters, metersZ: -halfMeters, rotation: rotation);
  ...
  void _drawGrid(Canvas canvas) {
    ...
    for (double meter = -halfMeters; meter <= halfMeters; meter += 1000) {
      canvas.drawLine(
        DioramaViewport.projectPerspectiveGround(metersX: meter, metersZ: -halfMeters, rotation: rotation),
        DioramaViewport.projectPerspectiveGround(metersX: meter, metersZ: halfMeters, rotation: rotation),
        paint,
      );
      canvas.drawLine(
        DioramaViewport.projectPerspectiveGround(metersX: -halfMeters, metersZ: meter, rotation: rotation),
        DioramaViewport.projectPerspectiveGround(metersX: halfMeters, metersZ: meter, rotation: rotation),
        paint,
      );
    }
  ...
  void _drawPoints(Canvas canvas, List<Offset> projected, List<GpsPoint> points) {
    ...
    for (int i = 0; i < projected.length; i++) {
      ...
      final double z = altitude.clamp(0, 250).toDouble() / 250 * 55;
  ...
  void _drawHeadingArrow(Canvas canvas, Offset origin, double heading, double length) {
    ...
    final Offset right = end + Offset(math.cos(angle - 2.5), math.sin(angle - 2.5)) * 8;
  ...
  void _drawScaleLegend(Canvas canvas, Size size, List<GpsPoint> points) {
    final TextPainter textPainter = TextPainter(
      text: TextSpan(
        text: 'Sessão: ${points.length} pontos · perspectiva rotacionável · área 10km×10km',
        style: const TextStyle(
  ...
  bool shouldRepaint(covariant DioramaSessionPainter oldDelegate) {
    return oldDelegate.points != points || oldDelegate.rotation != rotation;
  }
}
